...
노드는 여덟 가지입니다. 질문·답변(고정), **도구**(문서 검색·MCP·다른 워크플로우), **분기**(조건 참/거짓),
**모델 호출**(프롬프트로 모델을 부르고 글을 받음, 구조화 항목이면 JSON), **질문 분류**(갈래 중
하나로 분류해 그 갈래의 간선만 엶), **정보 추출**(글에서 항목을 뽑아 다음 노드의 인자로), **텍스트
조합**(앞 노드 값들을 글 하나로). 앞 노드의 값은 `{{노드.항목}}`으로 씁니다. 편집기에는 되돌리기·
다시하기·복제·자동 정렬과 단축키가 있고, 실행해보기가 노드마다 무엇을 냈는지 보여 줍니다. 설계와
실측은 [기술 보고서 §14](docs/technical-report.md).
