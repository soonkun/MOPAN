...
def _system_prompt(count: int) -> str:
    return (
        ...
        "keyword string.\n"
        "- Each line is SELF-CONTAINED and stays inside what the original asks. Never introduce a "
        "topic the original does not ask about.\n"
        "- IF THE QUESTION ASKS SEVERAL THINGS, give each its own line, as a whole question that "
        "stands alone without the others. One retrieval query cannot rank passages for three "
        "different things at once, and the parts are usually written up in different places. "
        "EVERY such line must still carry the question's SUBJECT MATTER - the goods, product, "
        "right or document it is about - or it retrieves the general rule instead of the answer.\n"
        "- REPLACE EVERY COLLOQUIAL, ABBREVIATED OR SPOKEN WORD with the term the corpus itself "
        "would print: 어플/앱 -> 애플리케이션 소프트웨어(컴퓨터 프로그램), 상표 이름 -> 상표견본, "
        "등록대상 -> 상표등록출원서의 기재사항, 류 -> 상품류(니스 분류). This is the substitution "
        "that matters most: an examination manual never uses the word the user typed, and no "
        "tokenizer bridges 어플 to 애플리케이션.\n"
        "- Otherwise what each line adds is a SYNONYM OR LEGAL-TERM VARIANT: the other name the "
        "corpus might use for the same office, statute, procedure, right or document - "
        "지식재산처/특허청, 거절결정/거절사정, 존속기간연장/연장등록, 출원인/신청인, "
        "이의신청/정보제공. Prefer one substitution family per line.\n"
